...
namespace AniSuki.View
{
    public partial class FrmBrowse : FrmBase
    {
        ...
        private void RefreshResolution()
        {
            ...
            LoadResolutions();
            if(null != oldRes) cmbFilterResolution.SelectedIndex = cmbFilterResolution.FindStringExact(oldRes.ResolutionString); // 还原原来选中的筛选项
        }
        private async void TaskSearch()
        {
            ...
            if(!string.IsNullOrWhiteSpace(FilterName)) filters.Add(string.Format($@"Name LIKE N'%{FilterName.Trim()}%'"));
            if(null != FilterProducerID) filters.Add(string.Format($@"ProducerID = {FilterProducerID.Value}"));
            if(null != FilterResolutionID) filters.Add(string.Format($@"ResolutionID = {FilterResolutionID.Value}"));
            if(null != FilterSaleDateAfter) filters.Add(string.Format($@"DATEDIFF(dd, SaleDate, '{FilterSaleDateAfter.Value.ToShortDateString()}') <= 0"));
            if(null != FilterSaleDateBefore) filters.Add(string.Format($@"DATEDIFF(dd, SaleDate, '{FilterSaleDateBefore.Value.ToShortDateString()}') <= 0"));
        ...
        private void BtnExplorer_Click(object sender, EventArgs e)
        {
            ...
            if(Directory.Exists(dir)) FileTool.OpenDirectory(dir);
            else MessageBoxEx.Error(@"目录不存在！");
        }
        private void BtnFreshResolution_Click(object sender, EventArgs e)
        {
            ...
            if(maxRes > resolution || maxRes < resolution)
            {
                ...
                if(-1 == cmbFilterResolution.FindStringExact(newRes.ResolutionString)) RefreshResolution(); // 新的分辨率，更新到筛选条件中
            }
